...
// split a string by delimiter and convert to int
...
// GC content (10% bin)
int gc_bin_from_unit(const string &unit){
	if(unit.empty()) return -1;
	int gc = 0;
	for(char c : unit){
		if(c == 'G' || c == 'C') gc++;
	}
	double pct = (100.0 * gc) / (double)unit.size();
	int ip = static_cast<int>(floor(pct + 1e-9));
	if(ip >= 100) return 100;
	if(ip < 0) ip = 0;
	return (ip / 10) * 10;
}
...
public:
	...
	void get_motifs(const TRGT_file &child, const map<string, int> &trid_map){
		for(const auto &trid_val : trid){
			int idx = trid_map.at(trid_val);
			vector<int> unit_gc;
			for(const auto &unit : child.motifs[idx]) unit_gc.push_back(gc_bin_from_unit(unit));
			motifs.push_back(unit_gc);
		}
	}
	...
	void count_unit_size(const TRGT_file &father, const TRGT_file &mother, const TRGT_file &child){
		// unit長の一覧を取得
		unit_count_rows.clear();
		for(int b = 0; b <= 100; b += 10) unit_count_rows.push_back(b);
		...
		// Statistics行の各unit_countはカバレッジ条件のみ，unit_count_dup/contはde novo条件（たぶん）
